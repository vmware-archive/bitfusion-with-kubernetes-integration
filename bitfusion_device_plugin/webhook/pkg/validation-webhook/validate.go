...
func (webhookServer *ValidateWebhookServer) Serve(w http.ResponseWriter, r *http.Request) {
	...
	if _, _, err := deserializer.Decode(body, nil, &ar); err != nil {
		...
		glog.Infof("Simon Logging: %v", pod.Spec.Containers)

		glog.Infof("AdmissionReview for Kind=%v, Namespace=%v Name=%v (%v) UID=%v patchOperation=%v UserInfo=%v",
			req.Kind, req.Namespace, req.Name, pod.Name, req.UID, req.Operation, req.UserInfo)

		admissionResponse = webhookServer.validate(&ar) // call some functions to check resource
	}
	...
	if admissionResponse != nil {
		glog.Infof("Simon admissionResponse: %v", admissionResponse)
		glog.Infof("Simon Request UID: %v", ar.Request.UID)
...
func (webhookServer *ValidateWebhookServer) validate(ar *v1beta1.AdmissionReview) *v1beta1.AdmissionResponse {
	...
	// status := annotations[admissionWebhookAnnotationInjectKey]
